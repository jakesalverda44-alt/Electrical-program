...
describe('ScaleCalibrationPopover', () => {
  it('computes ftPerPt from the typed known length and the two points\' distance', () => {
    ...
    render(<ScaleCalibrationPopover points={[{ x: 0, y: 0 }, { x: 100, y: 0 }]} titleBlockLabel={null} rawSuggestedFtPerPt={null} halfSize={false} onCommit={onCommit} onCancel={vi.fn()} />);
  ...
  it('the Set scale button is disabled until a valid length is typed', () => {
    render(<ScaleCalibrationPopover points={[{ x: 0, y: 0 }, { x: 100, y: 0 }]} titleBlockLabel={null} rawSuggestedFtPerPt={null} halfSize={false} onCommit={vi.fn()} onCancel={vi.fn()} />);
  ...
  it('offers a one-click "Use <title-block label>" button when a suggestion is available', () => {
    const onCommit = vi.fn();
    render(<ScaleCalibrationPopover points={[{ x: 0, y: 0 }, { x: 720, y: 0 }]} titleBlockLabel={`1/8" = 1'-0"`} rawSuggestedFtPerPt={1 / (0.125 * 72)} halfSize={false} onCommit={onCommit} onCancel={vi.fn()} />);
  ...
  it('Escape in the input calls onCancel', () => {
    ...
    render(<ScaleCalibrationPopover points={[{ x: 0, y: 0 }, { x: 100, y: 0 }]} titleBlockLabel={null} rawSuggestedFtPerPt={null} halfSize={false} onCommit={vi.fn()} onCancel={onCancel} />);
  ...
  describe('the >2% disagreement-with-the-title-block warning', () => {
    ...
    const rawSuggestedFtPerPt = 1 / (0.125 * 72);

    it('shows the warning when the typed measurement disagrees by more than 2%', () => {
      render(<ScaleCalibrationPopover points={points} titleBlockLabel={titleBlockLabel} rawSuggestedFtPerPt={rawSuggestedFtPerPt} halfSize={false} onCommit={vi.fn()} onCancel={vi.fn()} />);
    ...
    it('shows no warning when there is no title-block suggestion to compare against at all', () => {
      render(<ScaleCalibrationPopover points={points} titleBlockLabel={null} rawSuggestedFtPerPt={null} halfSize={false} onCommit={vi.fn()} onCancel={vi.fn()} />);
      fireEvent.change(screen.getByLabelText('Known length of this line:'), { target: { value: '12' } });
      expect(screen.queryByTestId('plan-scale-disagreement-warning')).toBeNull();
    });
    ...
    it('never blocks committing — Set scale stays enabled even while the warning shows', () => {
      const onCommit = vi.fn();
      render(<ScaleCalibrationPopover points={points} titleBlockLabel={titleBlockLabel} rawSuggestedFtPerPt={rawSuggestedFtPerPt} halfSize={false} onCommit={onCommit} onCancel={vi.fn()} />);
      fireEvent.change(screen.getByLabelText('Known length of this line:'), { target: { value: '12' } });
  ...
  describe('minimum calibration distance and extreme-scale warning', () => {
    it('Set scale is disabled, with an error, when the two points are closer than 50pt', () => {
      render(<ScaleCalibrationPopover points={[{ x: 0, y: 0 }, { x: 10, y: 0 }]} titleBlockLabel={null} rawSuggestedFtPerPt={null} halfSize={false} onCommit={vi.fn()} onCancel={vi.fn()} />);
      fireEvent.change(screen.getByLabelText('Known length of this line:'), { target: { value: '10' } });
    ...
    it('exactly 50pt is accepted (the boundary is inclusive)', () => {
      const onCommit = vi.fn();
      render(<ScaleCalibrationPopover points={[{ x: 0, y: 0 }, { x: 50, y: 0 }]} titleBlockLabel={null} rawSuggestedFtPerPt={null} halfSize={false} onCommit={onCommit} onCancel={vi.fn()} />);
    ...
    it('warns (but does not block) an implied scale that is unusually SMALL', () => {
      ...
      render(<ScaleCalibrationPopover points={[{ x: 0, y: 0 }, { x: 1000, y: 0 }]} titleBlockLabel={null} rawSuggestedFtPerPt={null} halfSize={false} onCommit={vi.fn()} onCancel={vi.fn()} />);
  ...
  // Fix round 2 / R2-B2 — the reviewer's exact B7-half-popover repro: on a
  // half-size document whose correct (as-printed) suggestion is 0.2222,
  // this popover used to commit 0.1111 (re-parsing the raw label text
  // with no half_size awareness at all), while the standalone banner —
  // reading the same raw value a different way — committed the correct
  // 0.2222. "Two buttons with the same label disagree."
  describe('half-size documents (R2-B2)', () => {
    const points: [{ x: number; y: number }, { x: number; y: number }] = [{ x: 0, y: 0 }, { x: 100, y: 0 }];
    const titleBlockLabel = `1/8" = 1'-0"`;
    const rawSuggestedFtPerPt = 1 / (0.125 * 72); // 0.11111...

    it('"Use <label>" commits the EFFECTIVE (doubled) scale on a half-size document, not the raw parse', () => {
      const onCommit = vi.fn();
      render(<ScaleCalibrationPopover points={points} titleBlockLabel={titleBlockLabel} rawSuggestedFtPerPt={rawSuggestedFtPerPt} halfSize onCommit={onCommit} onCancel={vi.fn()} />);
      fireEvent.click(screen.getByText(`Use 1/8" = 1'-0"`));
      expect(onCommit).toHaveBeenCalledWith(expect.closeTo(2 / (0.125 * 72), 10), titleBlockLabel);
    });

    it('"Use <label>" commits the RAW (un-doubled) scale on a normal-size document', () => {
      const onCommit = vi.fn();
      render(<ScaleCalibrationPopover points={points} titleBlockLabel={titleBlockLabel} rawSuggestedFtPerPt={rawSuggestedFtPerPt} halfSize={false} onCommit={onCommit} onCancel={vi.fn()} />);
      fireEvent.click(screen.getByText(`Use 1/8" = 1'-0"`));
      expect(onCommit).toHaveBeenCalledWith(expect.closeTo(rawSuggestedFtPerPt, 10), titleBlockLabel);
    });

    it('the 2% disagreement check compares against the EFFECTIVE (doubled) scale on a half-size document', () => {
      // A measurement of 0.2222 ft/pt (12ft over 54pt, roughly) agrees
      // with the DOUBLED suggestion (0.2222) but would show an ~8%
      // disagreement against the raw, un-doubled 0.1111 if the check
      // were still comparing against the raw value.
      render(<ScaleCalibrationPopover points={points} titleBlockLabel={titleBlockLabel} rawSuggestedFtPerPt={rawSuggestedFtPerPt} halfSize onCommit={vi.fn()} onCancel={vi.fn()} />);
      fireEvent.change(screen.getByLabelText('Known length of this line:'), { target: { value: '22.22' } }); // 22.22ft over 100pt = 0.2222 ft/pt
      expect(screen.queryByTestId('plan-scale-disagreement-warning')).toBeNull();
    });

    it('the 2% disagreement check WOULD warn if the raw (un-doubled) value were used instead — proving it actually uses the effective one', () => {
      render(<ScaleCalibrationPopover points={points} titleBlockLabel={titleBlockLabel} rawSuggestedFtPerPt={rawSuggestedFtPerPt} halfSize onCommit={vi.fn()} onCancel={vi.fn()} />);
      // A measurement matching the RAW (un-doubled) suggestion — a real
      // disagreement once half_size is accounted for.
      fireEvent.change(screen.getByLabelText('Known length of this line:'), { target: { value: '11.11' } });
      expect(screen.getByTestId('plan-scale-disagreement-warning')).toBeTruthy();
    });
  });
