...
interface ProjectState { phase: PhaseKey; installDate: string; notes: string; }
...
export default function GenProjectsPage({ gens, showToast }: Props) {
  ...
  const [states, setStates] = useState<Record<string, ProjectState>>(() =>
    Object.fromEntries(awarded.map(g => [g.id, {
      phase: (g.gen_install_phase as PhaseKey)||'scheduled', installDate:'', notes:''
    }]))
  );
  ...
  const salespeople = useMemo(() => Array.from(new Set(awarded.map(g => g.salesperson_name))).sort(), [awarded]);

  const loadProjectData = useCallback(async (id: string) => {
    const [coRes, fnRes] = await Promise.allSettled([
      api.get(`/projects/gen/${id}/change-orders`),
      api.get(`/projects/gen/${id}/field-notes`),
    ]);
  ...
  const ensureState = (g: Gen): ProjectState =>
    states[g.id] ?? { phase:(g.gen_install_phase as PhaseKey)||'scheduled', installDate:'', notes:'' };
  ...
  const TabContent = ({ gen }: { gen: Gen }) => {
    ...
    if (activeTab === 'overview') return (
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:24 }}>
        <div>
          <SectionLabel>Install Details</SectionLabel>
          <InfoGrid rows={[
            ['Customer',        gen.customer],
            ['Location',        gen.loc||'—'],
            ['Salesperson',     gen.salesperson_name],
          ...
          <div style={{ marginTop:16 }}>
            ...
            <input type="date" value={st.installDate}
              onChange={e => setStates(prev => ({ ...prev, [id]: { ...st, installDate:e.target.value } }))}
              style={INPUT}/>
          </div>
        </div>
        <div>
          ...
          <SectionLabel>Install Notes</SectionLabel>
    ...
    if (activeTab === 'equipment') {
      const sec = (secMap[id]?.equipment as any) ?? {};
      const [draft, setDraft] = useState<Record<string,string>>(sec);
      const fields = [
        { label:'Model Number',          key:'model'         },
        { label:'Serial Number',         key:'serial'        },
        { label:'Engine Serial',         key:'engine_serial' },
        { label:'Alternator Serial',     key:'alt_serial'    },
        { label:'Transfer Switch Model', key:'ats_model'     },
        { label:'Transfer Switch Serial',key:'ats_serial'    },
        { label:'Fuel Type',             key:'fuel_type'     },
        { label:'Warranty Expiration',   key:'warranty_exp'  },
      ];
      const save = async () => {
        await api.put(`/projects/gen/${id}/section/equipment`, { data: draft });
        setSecMap(p => ({ ...p, [id]: { ...(p[id]??{}), equipment: draft } }));
        showToast({ title: 'Equipment info saved' });
      };
      return (
        <div>
          <div style={{ display:'grid', gridTemplateColumns:'repeat(2,1fr)', gap:14, marginBottom:16 }}>
            <div style={{ background:'var(--surface2)', borderRadius:10, padding:'14px 16px', display:'flex', alignItems:'center', gap:12 }}>
              <span style={{ fontSize:28, fontWeight:900, color:'var(--amber)' }} className="num">{gen.kw}kW</span>
              <div>
                <div style={{ fontSize:11, fontWeight:700, color:'var(--text3)', textTransform:'uppercase', letterSpacing:'.05em' }}>Generator</div>
                <div style={{ fontSize:14, fontWeight:800, color:'var(--text)' }}>{gen.mfr} {gen.model}</div>
              </div>
            </div>
            <div style={{ background:'var(--surface2)', borderRadius:10, padding:'14px 16px' }}>
              <div style={{ fontSize:11, fontWeight:700, color:'var(--text3)', textTransform:'uppercase', letterSpacing:'.05em', marginBottom:4 }}>Contract</div>
              <div style={{ fontSize:18, fontWeight:900, color:'var(--text)' }} className="num">{moneyFull(gen.amount)}</div>
            </div>
          </div>
          ...
          <button className="btn" style={{ fontSize:13 }} onClick={save}>
            <Icon name="check" size={14} stroke={2}/> Save Equipment Info
          </button>
        </div>
      );
    }
      ...
      return (
        <div>
          <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:14, marginBottom:24 }}>
            {[
              { label:'Contract Value',    val:moneyFull(gen.amount),             color:'var(--blue)'  },
              { label:'Approved COs',      val:moneyFull(coTotal),                color:'var(--green)' },
              { label:'Revised Total',     val:moneyFull(gen.amount+coTotal),     color:'var(--text)'  },
            ...
            <div style={{ display:'grid', gridTemplateColumns:'2fr 1fr 1fr 1fr auto', gap:10, alignItems:'end' }}>
              ...
              <button className="btn" style={{ fontSize:13, whiteSpace:'nowrap' }}
                onClick={async () => {
                  ...
                  const res = await api.post(`/projects/gen/${id}/change-orders`, {
                    ...
                    <tr key={co.id}>
                      ...
                      <td>
                        <button onClick={async () => {
                          await api.delete(`/projects/gen/${id}/change-orders/${co.id}`);
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:10, marginBottom:10 }}>
            ...
            <div>
              ...
              <input value={fnForm.weather} onChange={e=>setFnForm(f=>({...f,weather:e.target.value}))}
                placeholder="e.g. Sunny 84°F" style={INPUT}/>
            </div>
            <div>
          ...
          <div style={{ display:'flex', gap:10, alignItems:'flex-end' }}>
            <div style={{ flex:1 }}>
              ...
              <textarea value={fnForm.note} onChange={e=>setFnForm(f=>({...f,note:e.target.value}))}
                placeholder="Installation progress, site conditions, issues encountered…"
                style={{ ...INPUT, height:72, resize:'vertical' }}/>
            </div>
            <button className="btn" style={{ fontSize:13, whiteSpace:'nowrap' }}
              onClick={async () => {
                if (!fnForm.note.trim()) { showToast({ title:'Note required' }); return; }
                const res = await api.post(`/projects/gen/${id}/field-notes`, {
            ...
            {fns.map(fn => (
              <div key={fn.id} className="panel" style={{ padding:'14px 18px' }}>
                <div style={{ display:'flex', justifyContent:'space-between', marginBottom:6 }}>
                  ...
                  <button onClick={async () => {
                    await api.delete(`/projects/gen/${id}/field-notes/${fn.id}`);
    ...
    if (activeTab === 'startup') {
      const sec = (secMap[id]?.startup as any) ?? {};
      const checks = [
        { label:'Fuel Level Verified',           key:'fuel_ok'         },
        { label:'Oil Level Checked',             key:'oil_ok'          },
        { label:'Coolant Level Checked',         key:'coolant_ok'      },
        { label:'Battery Voltage OK',            key:'battery_ok'      },
        { label:'Transfer Switch Tested',        key:'ats_tested'      },
        { label:'Load Test Completed',           key:'load_test'       },
        { label:'Automatic Start Verified',      key:'auto_start'      },
        { label:'Exercise Schedule Programmed',  key:'exercise_set'    },
        { label:'Customer Walkthrough Done',     key:'cx_walkthrough'  },
        { label:'Startup Sheet Signed',          key:'startup_signed'  },
      ];
      const [draft, setDraft] = useState<Record<string,boolean>>(sec);
      const [startNotes, setStartNotes] = useState<string>((sec.notes as string)||'');
      const done = checks.filter(c=>draft[c.key]).length;
      const save = async () => {
        await api.put(`/projects/gen/${id}/section/startup`, { data: { ...draft, notes:startNotes } });
        setSecMap(p => ({ ...p, [id]:{ ...(p[id]??{}), startup:{ ...draft, notes:startNotes } } }));
        showToast({ title:'Startup checklist saved' });
      };
      return (
        <div>
          <div style={{ marginBottom:16 }}>
            <div style={{ fontSize:13, fontWeight:700, color:'var(--text3)', marginBottom:10 }}>
              {done} / {checks.length} checks complete
            </div>
            <div style={{ height:6, background:'var(--surface2)', borderRadius:4, overflow:'hidden', marginBottom:18 }}>
              <div style={{ height:'100%', width:`${(done/checks.length)*100}%`, background:done===checks.length?'var(--green)':'var(--amber)', borderRadius:4, transition:'width .3s' }}/>
            </div>
            <div style={{ display:'grid', gridTemplateColumns:'repeat(2,1fr)', gap:10 }}>
              {checks.map(item => (
                <label key={item.key} style={{ display:'flex', alignItems:'center', gap:10, cursor:'pointer',
                  ...
                  color:draft[item.key]?'var(--green)':'var(--text2)' }}>
          ...
          <div style={{ marginBottom:12 }}>
            <FieldLabel>Startup Notes</FieldLabel>
            <textarea value={startNotes} onChange={e=>setStartNotes(e.target.value)}
              placeholder="Readings, issues found, settings adjusted…"
              style={{ ...INPUT, height:80, resize:'vertical' }}/>
          </div>
          <button className="btn" style={{ fontSize:13 }} onClick={save}>
            <Icon name="check" size={14} stroke={2}/> Save Startup Checklist
          </button>
        </div>
      );
    }
    ...
    if (activeTab === 'closeout') {
      ...
      const items = [
        { label:'Warranty Card Registered',   key:'warranty_reg'   },
        { label:'O&M Manual Delivered',       key:'om_delivered'   },
        { label:'As-Built Drawings Done',     key:'as_builts'      },
        { label:'Permit Closed',              key:'permit_closed'  },
        { label:'Lien Waiver Signed',         key:'lien_signed'    },
        { label:'Final Invoice Sent',         key:'invoice_sent'   },
        { label:'Payment Received',           key:'payment_recv'   },
        { label:'Customer Satisfaction',      key:'cx_satisfied'   },
      ];
      const [draft, setDraft] = useState<Record<string,boolean>>(sec);
      const [closeNotes, setCloseNotes] = useState<string>((sec.notes as string)||'');
      const done = items.filter(i=>draft[i.key]).length;
      const save = async () => {
        await api.put(`/projects/gen/${id}/section/closeout`, { data:{ ...draft, notes:closeNotes } });
        ...
        <div>
          ...
          <div style={{ marginBottom:12 }}>
            ...
            <textarea value={closeNotes} onChange={e=>setCloseNotes(e.target.value)}
              placeholder="Final items, outstanding issues…"
              style={{ ...INPUT, height:80, resize:'vertical' }}/>
          </div>
          <button className="btn" style={{ fontSize:13 }} onClick={save}>
    ...
    <div className="scroll view-enter">
      <div style={{ padding:'20px 28px 40px' }}>
        ...
        <div style={{ display:'flex', gap:10, alignItems:'center', marginBottom:16, flexWrap:'wrap' }}>
          {[
            { label:'Phase', val:filterPhase, set:setFilterPhase, opts:[['all','All Phases'],...PHASES.map(p=>[p.key,p.label])] },
            { label:'Rep',   val:filterRep,   set:setFilterRep,   opts:[['all','All Reps'],...salespeople.map(r=>[r,r])] },
            { label:'Brand', val:filterMfr,   set:setFilterMfr,   opts:[['all','All Brands'],['Kohler','Kohler'],['Generac','Generac']] },
          ].map(f => (
...
function StatusChip({ status }: { status: string }) {
  const colors: Record<string,string> = { pending:'#E0A53B', approved:'#34C588', rejected:'#E06A6A' };
