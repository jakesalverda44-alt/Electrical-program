...
function DocSlot({ genId, category, label }: { genId: string; category: string; label: string }) {
      ...
      <div className="builder-layout" style={{ display: 'grid', gridTemplateColumns: '1fr 300px', gap: 16, padding: '20px 28px 40px', alignItems: 'start' }}>
        <div>
          ...
          <Section title="Installation Options" icon="gear">
            ...
            <Field label="Electrical Feed Distance (ft)">
              <input type="number" min={0} style={INPUT_STYLE} value={form.feedFt} onChange={e => set('feedFt', Number(e.target.value))}/>
            </Field>
            <Field label="Generator — Side of House">
              <select style={SELECT_STYLE} value={form.genSide} onChange={e => set('genSide', e.target.value)}>
                <option value="">—</option>
                <option value="Left">Left</option>
                <option value="Right">Right</option>
              </select>
            </Field>
            <Field label="Position vs. Panel">
              <select style={SELECT_STYLE} value={form.panelRel} onChange={e => set('panelRel', e.target.value)}>
                <option value="">—</option>
                <option value="Same side as panel">Same side as panel</option>
                <option value="Opposite side of panel">Opposite side of panel</option>
                <option value="Next to panel">Next to panel</option>
              </select>
            </Field>
            {form.panelRel !== 'Next to panel' && (
              <Field label="Distance from Panel (ft)">
                <input type="number" min={0} style={INPUT_STYLE} value={form.panelFt} onChange={e => set('panelFt', Number(e.target.value))}/>
              </Field>
            )}
          </Section>
          ...
          <Section title="Job Documents" icon="doc">
            {savedGenId ? (
              <div style={{ gridColumn: '1 / -1', display: 'flex', flexDirection: 'column', gap: 8 }}>
                <DocSlot genId={savedGenId} category="sizer_report" label="Sizer Report"/>
                <DocSlot genId={savedGenId} category="survey" label="Survey"/>
              </div>
            ) : (
              <div style={{ gridColumn: '1 / -1', fontSize: 12, color: 'var(--text3)', fontWeight: 600 }}>
                Save the proposal first, then attach the sizer report and survey here.
              </div>
            )}
          </Section>
        </div>
