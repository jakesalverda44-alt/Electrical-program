...
export default function PreBidScopeCompare({ bidId, compId, compName, subjectSections }: {
  ...
  const { data, loading, error: loadError } = useApi<{ scope?: { sections?: PrebidSection[] } }>(
    `/preconstruction/${compId}/prebid`,
  );
  const compSections = data?.scope?.sections ?? null;
  const error = loadError ? 'Could not load the comparable\u2019s scope.' : null;
