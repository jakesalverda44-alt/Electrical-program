...
SCHEDULE QUANTITIES — a quantity that only a schedule gives is read from the schedule's own rows by a separate step: never put a quantity in quantities[] for panel-schedule circuits or breakers ("20/1 branch circuits", "lighting circuits") or for equipment-schedule items (battery chargers, water heater, drinking fountain …). List the equipment in equipment[] with its tag and description (include any printed count, e.g. "(5)", and the circuit numbers), and the lighting circuits in panelCircuits[].
