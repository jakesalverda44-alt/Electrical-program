...
/** True for our own cancellation and for the SDK's abort error. */
export function isCancellationError(err: unknown): boolean {
  if (err instanceof RunCancelledError) return true;
  const e = err as { name?: string; message?: string; constructor?: { name?: string } } | null;
  const text = `${e?.name ?? ''} ${e?.constructor?.name ?? ''} ${e?.message ?? ''}`;
  return /Abort|aborted/i.test(text);
}
...
export function registerRun(bidId: string, kind: RunKind): RunHandle {
...
/** Aborts every registered job of these kinds for the bid. Returns how many. */
export function abortRuns(bidId: string, kinds: RunKind[]): number {
  let n = 0;
  for (const kind of kinds) {
    for (const c of running.get(keyOf(bidId, kind)) ?? []) {
      if (!c.signal.aborted) { c.abort(new RunCancelledError('Stopped by the estimator')); n++; }
    }
  }
  return n;
}
